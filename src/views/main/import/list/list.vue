  ...
  <div class="capitalpool">
    ...
    >
      ...
      <template #right="scope">
        <div class="start-time">{{ scope.row.right_start_date }}</div>
        <div class="end-time">{{ scope.row.right_end_date }}</div>
      </template>
      ...
      <template #batchStatus_type="scope">
        <div v-if="scope.row.type == 1">导入会员</div>
        <div v-if="scope.row.type == 2">导入余额</div>
      </template>
    </page-content>
    <page-modal
      :defaultInfo="defaultInfo"
      ref="pageModalRef"
      :clickName="importlist"
